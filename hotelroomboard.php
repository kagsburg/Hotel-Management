...
<head>
   ...
   <title>Hotel Rooms Planning Board | Hotel Manager</title>
      ...
      <div id="wrapper">
         ...
         <div id="page-wrapper" class="gray-bg">
            ...
            <div class="row wrapper border-bottom white-bg page-heading">
               <div class="col-lg-10">
                  <h2>Hotel Rooms Board</h2>
                  <ol class="breadcrumb">
                     <li> <a href=""><i class="fa fa-home"></i> Home</a> </li>

                     <li class="active">
                        <strong>Hotel Rooms Board</strong>
                     </li>
                  </ol>
               </div>
               <div class="col-lg-2">
                        ...
                        <div class="ibox-content">
                           <div class="table-responsive">
                              <table class="table table-bordered table-striped">
                                 ...
                                 <tbody>
                                    <?php
                                    $getrooms = mysqli_query($con, "SELECT * FROM rooms WHERE status='1' ORDER BY roomnumber");
                                    while ($row =  mysqli_fetch_array($getrooms)) {
                                       ...
                                       // $getresertions = mysqli_query($con, "SELECT * FROM reservations WHERE room='$room_id' AND (arrivaltime >= '$start' AND arrivaltime < '$end') OR (departuretime >= '$start' AND departuretime < '$end')");
                                       // if (mysqli_num_rows($getresertions) > 0) {

                                       // }
                                       $rows = [[]];
                                       $getresertions = mysqli_query($con, "SELECT * FROM reservations WHERE room='$room_id' AND ((checkin >= '$start' AND checkin < '$end') OR (checkout >= '$start' AND checkout < '$end'))");
                                       ...
                                       while ($reservation = mysqli_fetch_array($getresertions)) {
                                          ...
                                          $arrival = $arrival < $start ? $start : $arrival;
                                    ...
                                    ?>
                                       <?php foreach ($rows as $rowidx => $row) : ?>
                                          <tr>
                                             <?php if ($rowidx === 0) { ?>
                                                <td rowspan="<?php echo count($rows); ?>" class="pd"><?php echo $roomtype; ?></td>
                                                ...
                                                ?>
                                                   <td colspan="<?php echo $cols; ?>">
                                                      <div class="resbar <?php echo status_color($ev["reservation"]); ?>">
                                                         <a href="reservation?id=<?php echo $rs["reservation_id"] ?>"><?php echo $ev["name"]; ?></a>
                                                      </div>
                                                   </td>
                                                <?php
                                                } else {
