...
<head>
   ...
   <title>Kitchen Stock Items | Hotel Manager</title>
      ...
      <div id="wrapper">
         ...
         <div id="page-wrapper" class="gray-bg">
            ...
            <div class="wrapper wrapper-content animated fadeInRight">
               <div class="row">
                  <div class="col-lg-12">
                     <!-- <a href="getstockreport" class="btn btn-sm btn-info pull-right">Get Report</a>
                     <a href="stockcsv" class="btn btn-sm btn-primary pull-right">Export to Excel</a>
                     <a href="stockprint" target="_blank" class="btn btn-sm btn-warning pull-right">Print PDF</a> -->
                     <div class="ibox float-e-margins">
                        <div class="ibox-title">
                           <h5>Kitchen Stock ITems <small>Sort, search</small></h5>
                        </div>
                              ...
                              <table class="table table-striped table-bordered table-hover dataTables-example">
                                 ...
                                 <tbody>
                                    ...
                                    while ($row =  mysqli_fetch_array($stock)) {
                                       ...
                                       $status = $row['status'];
                                       $getadded = mysqli_query($con, "SELECT SUM(quantity) as addedstock FROM kitchenstockitems WHERE item_id='$stockitem_id'") or die(mysqli_error($con));
                                       $rowa = mysqli_fetch_array($getadded);
                                       $addedstock = $rowa['addedstock'];
                                       $totalconsumed = 0;
                                       $getconsumeditems = mysqli_query($con, "SELECT * FROM consumeditems  WHERE stockitem_id='$stockitem_id'") or die(mysqli_error($con));
                                       while ($rowc = mysqli_fetch_array($getconsumeditems)) {
                                          $order_id = $rowc['order_id'];
                                          $quantity = $rowc['quantity'];
                                          $items = $rowc['items'];
                                          $consumed = $items * $quantity;
                                          $getorders = mysqli_query($con, "SELECT * FROM orders WHERE order_id='$order_id' AND status IN (1,2)") or die(mysqli_error($con));
                                          if (mysqli_num_rows($getorders) > 0) {
                                             $totalconsumed = $totalconsumed + $consumed;
                                          }
                                       }
                                       $stockleft = $addedstock - $totalconsumed;
                                    ?>
                                       <tr class="gradeA">
