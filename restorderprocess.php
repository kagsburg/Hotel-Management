...
if (isset($_POST['item_id'])) {
    ...
    if (empty($pquantity)) {
        ...
        while ($statement->fetch()) {
            ...
            $new_product["menuitem"] = preg_replace('/[^a-zA-Z0-9\-]/', ' ', $menuitem); //fetch product name from database
            $new_product["itemprice"] = $itemprice;  //fetch product price from database

            if (isset($_SESSION["rproducts"])) {  //if session var already exist
                if (isset($_SESSION["rproducts"][$new_product['item_id']])) //check item exist in products array
                {
                    unset($_SESSION["rproducts"][$new_product['item_id']]); //unset old item
                }
            }

            $_SESSION["rproducts"][$new_product['item_id']] = $new_product;    //update products with new item array	
        }

        $total_items = count($_SESSION["rproducts"]); //count total items
...
if (isset($_GET["remove_code"]) && isset($_SESSION["rproducts"])) {
    ...
    if (isset($_SESSION["rproducts"][$item_id])) {
        unset($_SESSION["rproducts"][$item_id]);
    }

    $total_items = count($_SESSION["rproducts"]);
