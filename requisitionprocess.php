...
if (isset($_POST['item_id'])) {
    ...
    $iprice = isset($_POST['price']) ? $_POST['price'] : 0;
    if ((empty($iquantity)) ) {
        ...
        while ($statement->fetch()) {
            ...
            if (isset($_SESSION["reqproducts"])) {  //if session var already exist
                if (isset($_SESSION["reqproducts"][$new_product['item_id']])) //check item exist in products array
                {
                    unset($_SESSION["reqproducts"][$new_product['item_id']]); //unset old item
                }
            }

            $_SESSION["reqproducts"][$new_product['item_id']] = $new_product;    //update products with new item array	
        }

        $total_items = count($_SESSION["reqproducts"]); //count total items
...
if (isset($_GET["remove_code"]) && isset($_SESSION["reqproducts"])) {
    $item_id   = filter_var($_GET["remove_code"], FILTER_SANITIZE_STRING); //get the product code to remove

    if (isset($_SESSION["reqproducts"][$item_id])) {
        unset($_SESSION["reqproducts"][$item_id]);
    }

    $total_items = count($_SESSION["reqproducts"]);
